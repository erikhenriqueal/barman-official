...
export default new Event('ready', __filename, async (client) => {
	...
	await client.rest.put(Discord.Routes.applicationCommands(client.application.id), { body: [] });

	const clientGuilds = await DiscordUtils.Cache.cacheGuilds(client);
	for (const guild of clientGuilds.values()) await client.rest.put(Discord.Routes.applicationGuildCommands(client.application.id, guild.id), { body: [] });
	...
	await client.rest.put(Discord.Routes.applicationCommands(client.application.id), { body: globalCommands.map((c) => c.toJSON()) });

	const guildedCommands = commands.filter(c => !globalCommands.includes(c));
	const guilds = commands.map(c => c.guilds).flat().uniques();
	for (const id of guilds) {
		await client.rest.put(Discord.Routes.applicationGuildCommands(client.application.id, id), { body: [] });
		const guildCommands = guildedCommands.filter(c => c.guilds.includes(id));
		await client.rest.put(Discord.Routes.applicationGuildCommands(client.application.id, id), { body: guildCommands.map((c) => c.toJSON()) });
	}
