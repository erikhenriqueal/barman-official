...
export default class Entity {
	...
	constructor(data: EntityConstructorData, options?: EntityConstructorOptions) {
		...
		this.subprocess.on('spawn', () => log(`[ Entity - ${data.label} ] Child Process sucessfully spawned command '${data.command}' in process ${this.subprocess.pid}`, true))
		this.subprocess.on('close', (code, signal) => log(`[ Entity - ${data.label} ] Process closed.`, true))
		this.subprocess.on('disconnect', () => log(`[ Entity - ${data.label} ] Process disconnected.`, true))
		this.subprocess.on('error', err => log(displayText(`Received an Error:\n${err}`), true))
		this.subprocess.on('exit', (code, signal) => log(`[ Entity - ${data.label} ] Application exited with code ${code} (signal: ${signal})`, true))
		
		this.subprocess.stdout.on('data', chunk => log(displayText(parseChunk(chunk)), true))
		// this.subprocess.stdout.on('close', () => log(`[ Entity - ${data.label} ] 'stdout' closed.`, true))
		// this.subprocess.stdout.on('end', () => log(`[ Entity - ${data.label} ] 'stdout' ended.`, true))
		// this.subprocess.stdout.on('error', err => log(displayText(`Received an 'stdout' Error:\n${err}`), true))
		...
		// this.subprocess.stderr.on('close', () => log(`[ Entity - ${data.label} ] 'stderr' closed.`, true))
		// this.subprocess.stderr.on('end', () => log(`[ Entity - ${data.label} ] 'stderr' ended.`, true))
		// this.subprocess.stderr.on('error', err => log(displayText(`Received an 'stderr' Error:\n${err}`), true))
